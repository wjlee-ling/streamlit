...
class LinkHandler(BaseCallbackHandler):
    """
    Retrieval 검색 결과 링크 스트링 대체가 있을 때 메타데이터에 있는 링크 주소를 다시 삽입함
    """

    @property
    def vectorstore(self, vectorstore_of_bot, source_data_directory):
        self.source_data_directory = source_data_directory
        return vectorstore_of_bot

    # def _get_directory(self):
    #     return self.vectorstore._persist_directory

    def get_content_from_link(self, link):
        """Notion이 자동으로 생성한 하이퍼링크에 접속해서 해당 문서의 내용을 가져옴"""
        if not os.path.exists(self.source_data_directory):
            raise FileNotFoundError(f"{self.source_data_directory} does not exist.")

        decoded_url = os.path.join(self.source_data_directory, unquote(link))
        retrieved = self.vectorstore.get(
            where={"source": decoded_url}, include=["documents"]
        )  # can also get "embeddings", "metadatas", "documents", "ids"
        retrieved = " ".join(retrieved["documents"])
        return retrieved
    ...
    def on_retriever_end(
        ...
        **kwargs,
        ...
        """
        검색된 문서에 `(link@{idx})`가 있을 때, 해당 링크 문서 길이가 30 미만이면 괄호내로 삽입하고 이상이면 문서 맨 아래 추가하여 최종 doc_chain의 LLM으로 넘김
        """
        for doc in documents:
            if doc.metadata.get("links") != "":
                links = doc.metadata["links"].split("^")
                for idx, link in enumerate(links):
                    if f"(link@{idx})" in doc.page_content:
                        link_content = self.get_content_from_link(link)
                        if len(link_content) < 30:
                            doc.page_content = doc.page_content.replace(
                                f"(link@{idx})", f"({link_content})"
                            )
                        else:
                            doc.page_content = (
...
class NotionBot:
    ...
    def __call__(self, question: str):
        response = self.chain(question, callbacks=[LinkHandler()])
